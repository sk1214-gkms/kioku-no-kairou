...
- [**実機テストチェックシート**](docs/実機テストチェックシート.md) … ★自宅PCで一発で回す用。起動→通し→7結末到達→今回修正の確認→エッジケース
- [**3D制作マニュアル（初心者向け）**](docs/3D制作マニュアル_初心者向け.md) … 見ながら背景を作る（再掲）
- [**改善ロードマップ（採用決定4件）**](docs/改善ロードマップ.md) … ★30号室の伏線化(済)／審判やり直し＋分岐反応する部屋＋結末ヒント／収益ハイブリッド¥480確定／多言語方針（アートに文字を焼かない規則）
...
- [**ストーリー分岐フロー図**](docs/story_flow.drawio) … draw.io。★v2シート＝**ゲーム全体フロー**（13室の謎解法×決断フラグ×周回反応→審判の証拠ゲート→結末決定木→7結末＋審判やり直し/Bループ/タイマーD）／v1シート＝作話システム概念図
...
- [**★2Dイラスト制作マニュアル（Gemini・見ながら作る）**](docs/2Dイラスト制作マニュアル_Gemini.md) … ★アート方針＝2Dイラスト確定。基準1室→"同じ部屋を編集"で4方向/差分→色統一。一貫性の保ち方
- [3D制作マニュアル（旧方針・参考）](docs/3D制作マニュアル_初心者向け.md) … Blenderで作る場合の手順（現方針は2Dイラスト）
- [タイトル画面デザイン案（HTMLモック）](docs/design_title_mock.html) … ブラウザで開く単一HTML。配色はアプリ実装HEXと一致＝Flutter title_screen 実装の参考（実装済）
- [アプリアイコン導入手順](docs/アプリアイコン導入.md) … 採用=V3(頭文字A×鍵穴)。1024px→flutter_launcher_iconsで全サイズ生成（自宅）
- [背景の作り方：MMDステージ→Blender→レンダリング](docs/Blender制作手順_MMD取り込み.md) … 取り込みの技術詳細（プラグイン・カメラ・ライト）
