...
## ドキュメント
- [仕様書 v2.0](docs/仕様書_v2.md) … 改訂版・正本
- [構成案 v1（謎と物語）](docs/構成案_謎と物語_v1.md) … 館＝記憶の設計思想
- [ストーリー分岐フロー図](docs/story_flow.drawio) … draw.io（confront/推理/7エンディング 完全版）
- [アセット仕様書](docs/アセット仕様書.md) … **必要な画像/イラスト/音の詳細一覧（部屋ごと）**
...
- 対応：**タイトル/モード選択**、**ノーマル**／**タイマー（90秒・時間切れで虫食い）**／**ハード**、**全30部屋の骨格（St.1〜29＋最終室30）**、隠しアイテム、最終室の任意推理、A/A＋/A′/B/B′/C/C′ 分岐、**永続セーブ＋つづきから**、**広告の土台**（既定オフ）
  - 第1章(1-10)はノーマル＋ハード。**第2-3章(11-29)はノーマルのみ**（ハードは _solution フォールバックで動作。難化は後追い）。5/15/25 は選択分岐（逃避⇄直面）。
