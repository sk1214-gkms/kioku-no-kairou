...
- ☐ **分岐反応する部屋**：show_when/hide_when で選択に反応するディテールを6箇所前後（JSONのみ・フレーバー差）
- ☐ **分岐で“謎”が変わる部屋（3室）**：R10←R3①／R7←R6①／R5←R4分岐 で錠オブジェクトをフラグ出し分け（答えは不変・道筋だけ歪む）。JSONのみ・まず1室パイロット。解答キー＋ヒント変種も追記
