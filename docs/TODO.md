...
> 🧪 **仮画像で今すぐ“画像ありテスト”**：`python tools/gen_placeholders.py`（背景52枚→`assets/images/rooms/`）＋`python tools/gen_item_icons.py`（アイテム透過アイコン15個→`assets/images/items/`）を実行。コード生成なので**画風が必ず揃う**。本番アートは**同じパスに上書き**すれば差し替わる。※PNGは非追跡(.gitignore)なので各PCで実行。
...
## 4.5 🟡 採用済み改善（詳細は [改善ロードマップ](改善ロードマップ.md)）
> 前提：**自宅ビルド確認（legacy撤去 6e4b86b）が通ってから**コードに着手。
- ☑ 「30号室」の欠落を伏線化（judgment.json に一行追加・データのみ）
- ☐ A3：結末コレクションに条件ティーザー表示（小・title_screen）
- ☐ **審判やり直し**：審判突入時スナップショット＋「最後の審判からやり直す」（小〜中）※A1追想モードは不採用に変更
- ☐ **分岐反応する部屋**：show_when/hide_when で選択に反応するディテールを6箇所前後（JSONのみ）
- ☐ 収益ハイブリッド：リワード維持＋**プレミアム買い切り¥480（確定）**＋強制広告は章間のみ（マネタイズ期）
- ⚠ 恒久規則：**本番アートに"読ませる文字"を焼き込まない**（多言語対応のため。エンジン側で重畳）
